...
public class TranslatedNG20ClassificationWithEnglishIndex {
...
//		NGData data = readData(MultiLingualResourcesConfig.multiLingual20NGFolder + "ha");
//		test (data, 1);
	}
	...
	public static double test (NGData data, int topK, HashMap<String, HashSet<String>> dictionary) {
		...
		String indexFolderName = MultiLingualResourcesConfig.englishESA530IndexFolder;
		...
		tree.setDebug(false);
		tree.esa = new MultiLingalESA("en", new String[] {"text", "title"}, indexFolderName);
		...
		for (int j = 0; j < data.labeledContent.size(); ++j) {
			...
			System.out.println("[Classifying] doc " + j);
