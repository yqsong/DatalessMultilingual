...
public class TEDSupervisedClassificationWithSubSet {
	...
	public static void main (String args[]) {
		...
		double sampleRate = 0.7;
		try {
			for (int i = 0; i < TEDData.lgNameArray.length; ++i) {
				int lgID = i;
				
				if (lgID % fold == selected) {
					List<Double> f1MeanList = new ArrayList<Double>();
					List<Double> f1StdList = new ArrayList<Double>();
					for (int j = 0; j < TEDData.labelNameArray.length; ++j) {
						List<Double> f1SampledList = new ArrayList<Double>();
						for (int k = 0; k < 10; ++k) {
							double f1 = testOneLabelOneLanguage (lgID, j, 10000, 0.01, solver, sampleRate);
							f1SampledList.add(f1);
						}
						double mean = StatUtils.listAverage(f1SampledList);
						double std = StatUtils.std(f1SampledList, mean);
						
						f1MeanList.add(mean);
						f1StdList.add(std);
					}
					for (int j = 0; j < TEDData.labelNameArray.length; ++j) {
						System.out.println("[Summary] " + "\t" + TEDData.lgNameArray[lgID] + "\t" + TEDData.labelNameArray[j] 
								+ "\tMean: " + f1MeanList.get(j) + "\tStd: " + f1StdList.get(j));
					}
					
					double mean = StatUtils.listAverage(f1MeanList);
					double std = StatUtils.std(f1MeanList, mean);
...
//			}
	...
	public static double testOneLabelOneLanguage (int lgID, int labelID, double C, double eps, SolverType solver, double sampleRate) throws Exception {
